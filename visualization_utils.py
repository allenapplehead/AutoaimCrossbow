...
def visualize_boxes_and_labels_on_image_array(
        ...
        if not skip_labels:
          if not agnostic_mode:
            if classes[i] in six.viewkeys(category_index):
              ...
              class_name = f"{objCount}_{category_index[classes[i]]['name']}"
            else:
              class_name = 'N/A'
            ...
            xmin = int(xmin * width)
            xmax = int(xmax * width)
            objectsDetected.append([str(class_name), xmin, ymin, xmax, ymax])
  ...
  # MODIFIED LINE for Autoaim Crossbow
  # Note that returning objectsDetected will BREAK tensorflow evaluation
  # So when evaluating your model just comment the line right below out
  # so that it returns image
